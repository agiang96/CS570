...
//printf("after parse \n");
        ...
        if(outFlag != 0) { //for output file
            if(argCount == 0) { //if no words in newargv
                ...
                if(outFlag > 2) {
                    fprintf(stderr, "Ambiguous behavior. Failed to open writing file \n");
                    continue;
                }
...
//printf("inFlag: %d argCount: %d", inFlag, argCount);
            if(argCount == 0) { //if no words in newargv
                fprintf(stderr,"Missing name for redirection \n");
                continue;
            }
            else if(INFILE == '\0'){ //no OUTFILE specified
                fprintf(stderr, "Missing infile for redirection \n");
                continue;
            }
            else {
                if(inFlag > 2) {
                    fprintf(stderr, "Ambiguous behavior. Failed to open reading file\n");
                    continue;
                }
...
//printf("after in/out flag checks \n");
    //********************** cd ***********************
        ...
        if(strcmp(newargv[0], "MV") == 0) {
            ...
            if(argc > 3)
                ...
                if((mvlink = link(newargv[1],newargv[2])) != 0)
                    fprintf(stderr, "%s: cannot move '%s': No such file or directory\n",
...
int parse(){
    ...
    newargv[argCount] = '\0';
    if(strlength != 0 && argvWord == 0) return 0; //empty
    else if(strlength == 0 && argvWord == 0) return -10; //EOF
